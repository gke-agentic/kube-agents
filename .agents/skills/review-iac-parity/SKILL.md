...
- **Admission webhooks** are off in chart installs (`ENABLE_WEBHOOKS=false`) because the chart ships no cert-manager wiring. The script/kustomize path provides them. Documented in the chart README's Notes.
- **`modelProvider: chatgpt`** is rejected by the chart: it needs the OAuth-token PVC from the kustomize overlay.
- **Autopilot vs Standard.** The `gke-cluster` module builds an Autopilot cluster; `provision_01_gcp_cluster.sh` builds a Standard one. Everything node-level therefore has no Terraform counterpart — machine type, the gVisor node pool (`provision_02`), the managed-OTel scope.
...
- **The GitHub minter's PEM import** stays with `provision_10_deploy_github_minter.sh`; the `github-minter` module creates IAM and KMS only.
