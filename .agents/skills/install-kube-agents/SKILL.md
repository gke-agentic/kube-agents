...
When driving the installer on `main` (development only), `--image-tag` defaults to the local `HEAD` commit SHA and requires candidate images built for that commit in GHCR:
...
| `--image-tag=TAG`                    | SemVer release tag or full 40-character commit SHA                                                                                                                                                                                                     | Inferred from baked release in release installer/bundles, or local `HEAD`; required via generic `curl \| bash` |
