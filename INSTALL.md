...
2. Runs the same pre-flight checks a real run does — the schedulable-capacity preflight first, then the existing-cluster node-pool and NetworkPolicy consent gates, and the refusal for a cluster that cannot be described — without creating or modifying GCP resources. A cluster whose untainted nodes cannot fit the workloads and whose pools cannot scale to fit them is refused with `REFUSED_INSUFFICIENT_CAPACITY` (bypassable via `--skip-capacity-check`); one that needs `--migrate-node-pools` or `--enable-network-policy` is refused here too, exiting 1 with a `REFUSED_*` status. `install.env` and `terraform.tfvars` are written before these checks run, so a refused run leaves both on disk; what it withholds is the operator handoff and the `GENERATE_ONLY_SUCCESS` report, and the tfvars it leaves behind have not been validated.
...
- **`HELM_TIMEOUT`**: Timeout in seconds for Helm releases during rollout (default `600`, valid range `540`-`899`). Configurable via `--helm-timeout=SECONDS` or `HELM_TIMEOUT` in `install.env`. When a rollout times out, diagnostics inspect pending pods and scheduling warning events before failing.
