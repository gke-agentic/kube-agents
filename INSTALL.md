...
- **`SKIP_CAPACITY_CHECK`**: By default, `install.sh` verifies schedulable CPU and memory capacity on untainted nodes (and on `gvisor-pool` when present) before Terraform apply on existing Standard clusters (both initial install and Day-2 control panel Save & Apply). Pass `--skip-capacity-check` / `SKIP_CAPACITY_CHECK=true` to bypass this check if you plan to provision or resize nodes later.
- **`HELM_TIMEOUT`**: Wait timeout in seconds for the two Helm releases the full-install composition creates, `cert_manager` and `kube_agents` (default `600`, accepted range `540`-`899`; outside it the run aborts rather than clamping). Configurable via `--helm-timeout=SECONDS` or `HELM_TIMEOUT` in `install.env`. A full `./upgrade.sh` — the default mode — validates `HELM_TIMEOUT`, regenerates `terraform.tfvars` from `install.env`, and reapplies the composition, so it governs both releases there too; the chart-only modes (`--upgrade-mode=operator`, `--upgrade-mode=harness`) retag the live release with `helm upgrade` and wait a fixed 10 minutes instead. Both `install.sh` and `upgrade.sh` validate the value before running Terraform, and the `helm_timeout` variable's own validation rejects out-of-range values on a hand-driven `terraform plan`. Where both ends of the range come from is in [`terraform/examples/full-install/README.md`](terraform/examples/full-install/README.md#helm-rollout-timeout-helm_timeout), which owns that rationale. When a rollout times out, diagnostics inspect pending pods, unready or crash-looping `Running` pods, and scheduling warning events before failing — fetching target cluster credentials when `kubectl` has no current context, and skipping the diagnosis when `kubectl`'s current context points at another cluster (or falls back to in-cluster credentials) rather than reporting another cluster's pods under this one's failure.
