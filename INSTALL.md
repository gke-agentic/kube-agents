...
- **`MIGRATE_NODE_POOLS` & `ENABLE_NETWORK_POLICY`**: On existing clusters, migrating node pools to `GKE_METADATA` and enabling legacy Calico NetworkPolicy can recreate nodes and restart workloads. Pass `--migrate-node-pools` / `MIGRATE_NODE_POOLS=true` and `--enable-network-policy` / `ENABLE_NETWORK_POLICY=true` to authorize these mutations; otherwise they are skipped.
