...
echo "The script will perform the setup in the following project: $PROJECT_ID"
echo ""
...
for role in roles/cloudkms.admin roles/container.admin roles/iam.serviceAccountTokenCreator roles/serviceusage.serviceUsageAdmin roles/serviceusage.serviceUsageConsumer; do
...
gcloud iam workload-identity-pools providers create-oidc "${PROVIDER_NAME}" \
  ...
  --project="${PROJECT_ID}" || echo "Provider may already exist, continuing..."
...
echo "Granting Service Usage Consumer to the WIF Principal..."
gcloud projects add-iam-policy-binding "${PROJECT_ID}" \
