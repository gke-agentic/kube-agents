...
4. **[provision_04_gcp_k8s_secrets.sh](scripts/provision_04_gcp_k8s_secrets.sh)**:
   - Prompts for or reads the `MODEL_PROVIDER` and corresponding `GEMINI_API_KEY` or `ANTHROPIC_API_KEY`.
...
1. The `platform-agent-secrets` Secret created in your destination namespace (containing `GEMINI_API_KEY` or `ANTHROPIC_API_KEY`).
