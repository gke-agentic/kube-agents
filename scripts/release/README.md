...
- `resolve_deploy.sh`: Resolves candidate target commit SHA, validates lease policy, and normalizes force_reconcile parameters for long-lived environments (`autopush` and `staging`). Configured via `TARGET_ENVIRONMENT` environment variable (`autopush` or `staging`). Called by `autopush-deploy.yml` and `staging-deploy.yml`.
