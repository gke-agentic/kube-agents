...
Three settings on the `rc` GitHub environment turn it on, and all three must be present before the minter is provisioned at all ([`installer_common.sh`](../../k8s-operator/scripts/installer_common.sh)). All three empty is a supported configuration — an install without a minter, which is the default everywhere outside the RC. Some set and some empty is not: `provision_rc_environment.sh` exits non-zero rather than provisioning an RC whose token-minting test would fail with an HTTP 502 forty minutes later.

Put them on the **environment**, not the repository. A repository-scope value is invisible to the `deploy-rc` job and arrives empty, which is indistinguishable from never having set it. `GH_APP_ID` is an environment _secret_, so it needs one more thing that a variable does not: `rc-release-pipeline.yml` must call the deploy workflow with `secrets: inherit`. A `uses:` job cannot declare an environment, so an explicit `secrets:` mapping there resolves against nothing and forwards an empty string — which is exactly how the minter was silently skipped on every RC run until [#1027](https://github.com/gke-labs/kube-agents/pull/1027). `tests/test_rc_minter_secret_wiring.py` pins both halves.
