...
- `publish_github_release.sh`: Publishes official GitHub Releases with auto-generated release notes from Conventional Commits, discovers and attaches all distribution artifacts (`.tar.gz`, `.zip`, `.tgz`, `*.spdx.json`, `*.cdx.json`, `checksums.txt`) from `DIST_DIR`, and handles idempotent re-runs via `gh release upload --clobber`.
