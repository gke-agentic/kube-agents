...
- `provision_rc_environment.sh`: Tears the RC environment down with `uninstall.sh`, then reinstalls it at the candidate commit with `install.sh`, against the dedicated RC GCP project. It branches on `uninstall.sh`'s exit code (`./uninstall.sh --help` lists them); a failed teardown raises an `::error` annotation and a job-summary entry carrying the teardown output, and provisions anyway unless the `RC_TEARDOWN_STRICT` repository variable is `true`. **The teardown does not currently run**: `rc-deploy-environment.yml` installs no terraform before this step, and `install.sh` auto-installs it only afterwards, so every run reinstalls over the surviving environment. Whether to fix that by installing terraform first — committing the pipeline to a GKE cluster rebuild per run — or by resetting the workloads on a persistent cluster is an open decision.
