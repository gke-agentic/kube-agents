...
- **Scheduled Cadence (every 3 hours `17 */3 * * *`, best-effort)**:
  ...
  - **Redundant Run Skipping**: If the latest candidate commit already carries an `rc_*_validated` tag or was previously attempted, the pipeline skips subsequent provisioning and E2E test execution (`skip_rc=true`), finishing in seconds.
...
at that commit, runs the `nightly-e2e` matrix on it, tags the commit `staging_<ts>_<sha>` if the
...
It reuses the RC pipeline's machinery unchanged. `rc-deploy-environment.yml`,
`rc-teardown-environment.yml` and `e2e-run.yml` each take a `github_environment` input that renders
...
it — a job bound to an environment that does not exist resolves every `vars.*` to empty.

1. **A GCP project of its own.** Not `kube-agents-rc`: sharing the project would put the two
   pipelines back on one cluster, which is the collision this exists to remove.
...
3. **A `nightly` GitHub environment** holding the same variables as `rc` rather than a trimmed
   ...
   `GCP_PROJECT_ID`, `GCP_REGION`, `GKE_CLUSTER_NAME`, `GCP_WORKLOAD_IDENTITY_PROVIDER`,
   `GCP_SERVICE_ACCOUNT`, `REGISTRY_PREFIX`, `AGENT_NAMESPACE` and `RC_TEARDOWN_STRICT` are the
   ones the pipeline reads directly; the rest are install inputs.

### Integrations the nightly matrix needs and the RC does not

`nightly-e2e` is a superset of `rc-e2e`, so the nightly environment needs everything the RC
...
| `rc-deploy-environment.yml`   | Step 2 - Deploy Environment             | `resolve_rc_tag.sh`, `validate_and_log_deploy_summary.sh`, `provision_environment.sh`                                                                                                          |
| `e2e-run.yml`                 | Step 3 - GKE Readiness & E2E Validation | `install_e2e_deps.sh`, `install_pubsub_platform.sh`, `wait_for_gke_readiness.sh`, `execute_e2e_tests.sh`                                                                                       |
| `rc-tag-validated.yml`        | Step 4 - Validate Candidate Commit      | `resolve_rc_tag.sh`, `tag_validated_release.sh`                                                                                                                                                |
| `rc-teardown-environment.yml` | Step 5 - Tear Down Environment          | `resolve_rc_tag.sh`, `teardown_environment.sh`                                                                                                                                                 |
| `nightly-pipeline.yml`        | Nightly promotion to staging            | `resolve_promotion_candidate.sh`, `verify_candidate_images.sh`, `tag_staging_promotion.sh`, plus the three shared workflows above                                                              |
