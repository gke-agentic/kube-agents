...
| `check_existing_cluster_capacity_preflight`                               | Preflights schedulable capacity on untainted and `gvisor-pool` nodes (Standard)        |
...
One key is file-only: the front doors clear a shell-exported `NAMESPACE` before
reading the file because kubectl tooling exports that name and the value now reaches
the Helm release's namespace. The dev tooling's `load_state` does the same.
`KUBE_AGENTS_INSTALL_ENV` points at a different path, which is how CI renders one from
its own variables rather than keeping install state on an ephemeral runner. The front
doors also clear a shell-exported `HELM_TIMEOUT` before reading `install.env` (the file
and `--helm-timeout` are the two routes in).
