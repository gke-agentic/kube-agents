...
on a Standard cluster; `MEMORY` absent destroys the Hindsight API and its Postgres;
`ENABLE_GKE_BACKUP_PLAN` absent destroys the backup plan; `ENABLE_PUBSUB_PLATFORM` and
`ENABLE_STOCKOUT_INVESTIGATOR` absent destroy their Pub/Sub topics, subscriptions and IAM
grants; `GOOGLE_CHAT_ENABLED` absent removes the Chat topic and subscription;
`PLATFORM_AGENT_PERMISSION_SET` absent falls back to `read-only` and drops the custom roles.
