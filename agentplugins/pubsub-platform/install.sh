...
# If the AgentPlugin is already owned by another release (e.g. the main kube-agents chart),
# update Helm ownership metadata so this standalone release can upgrade it without collision.
if kubectl --context="$CONTEXT" get agentplugin pubsubplatform -n "$NAMESPACE" >/dev/null 2>&1; then
    ...
    if [ -n "$current_rel" ] && [ "$current_rel" != "pubsubplatform" ]; then
        echo "Adopting AgentPlugin pubsubplatform from release '${current_rel}' to 'pubsubplatform'..."
        kubectl --context="$CONTEXT" annotate agentplugin pubsubplatform -n "$NAMESPACE" meta.helm.sh/release-name="pubsubplatform" --overwrite >/dev/null 2>&1 || true
        kubectl --context="$CONTEXT" annotate agentplugin pubsubplatform -n "$NAMESPACE" meta.helm.sh/release-namespace="$NAMESPACE" --overwrite >/dev/null 2>&1 || true
        kubectl --context="$CONTEXT" label agentplugin pubsubplatform -n "$NAMESPACE" app.kubernetes.io/managed-by="Helm" --overwrite >/dev/null 2>&1 || true
    fi
fi
