...
1. **Mode** (part of key for node-level facets): `autopilot` when `.autopilot.enabled == true`, else `standard`. Autopilot and Standard clusters are **never** comparable for node-level facets; on Autopilot clusters, the five node-level facets marked _Standard cohorts only_ belong in `checks_not_applicable` rather than splitting cluster-level cohorts by mode.
