...
find agents/platform/skills -maxdepth 2 -name "SKILL.md" -exec awk 'NR==1,/^---$/ {if ($0!~/^---$/) print FILENAME ": " $0}' {} +
...
- **Specific Triggering Criteria:** `description` must clearly state *when* to use the skill and *what* specific tools/resources it acts on. Reject conversational fluff like `"Helpful guide for doing things"` in favor of `"Use when designing ComputeClasses and node selection strategies prior to deployment."`
...
grep -oE "\./[a-zA-Z0-9_-]+/.*\.py" agents/platform/skills/*/SKILL.md | while read -r match; do
