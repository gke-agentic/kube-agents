...
- Ensure the `hermes_otel` plugin is enabled in the profile's own config (on a `None` cluster, `enabled: false` and `backends: []` is expected) — `/opt/data/config.yaml` for the Chat Agent, `/opt/data/profiles/<profile>/config.yaml` for the Platform and Cluster Agents.
