...
`/opt/data/profiles/platform/skills/github-issue-resolver/scripts/resolver.py`. The LLM's
...
```bash
python3 /opt/data/profiles/platform/skills/github-issue-resolver/scripts/resolver.py poll
```
...
```bash
python3 /opt/data/profiles/platform/skills/github-issue-resolver/scripts/resolver.py claim --issue <number>
```
...
2. **Execute the deterministic transition script:** The script safely uploads
   ...
   - **Case A: Issue Resolved / False Alarm (`status:resolved`)**:
     ...
     ```bash
     python3 /opt/data/profiles/platform/skills/github-issue-resolver/scripts/resolver.py transition --issue <number> --state resolved --report-file /opt/data/scratch/report_<number>.md
     ```
     - Your final turn response MUST BE exactly `[SILENT]`.

   - **Case B: Human Review / SRE Action Needed (`status:escalation-needed`)**:
     ```bash
     python3 /opt/data/profiles/platform/skills/github-issue-resolver/scripts/resolver.py transition --issue <number> --state escalation-needed --report-file /opt/data/scratch/report_<number>.md
     ```
...
1. **Deterministic Transition Called:** `/opt/data/profiles/platform/skills/github-issue-resolver/scripts/resolver.py transition` was executed
