...
| The release gate                                                                           | `tests/e2e/`                                                                                     | `rc-release-pipeline.yml` three-hourly, `staging-promote.yml` nightly via `e2e-nightly-matrix.yml`, and `e2e-gchat-test.yml` / `e2e-manual-runner.yml` / that matrix by hand | nothing — it gates releases, not pull requests                                                                    |
...
**`tests/e2e/` is not manual-only.** `rc-release-pipeline.yml` runs the `rc-e2e` subset on
`cron: "17 */3 * * *"`, and `step-4-tag-validated` depends on it. Breaking a test there is not free
— it reds the release-candidate pipeline within three hours and stops the tag. `staging-promote.yml`
then calls `e2e-nightly-matrix.yml` at 02:00 UTC to run the whole `nightly-e2e` matrix against the
newest validated candidate, and breaking a test only that matrix reaches costs the night's promotion
to staging instead. The manual `e2e-gchat-test.yml` and `e2e-manual-runner.yml` are further callers,
not the only ones.
`tests/e2e/operator/agentplugins_e2e_test.py` has no scheduled caller but that nightly matrix: no
three-hourly run and no pull request touches it, and the rest is hand-started dispatch.
