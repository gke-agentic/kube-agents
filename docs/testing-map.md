...
| The release gate                                                                           | `tests/e2e/`                                                                                     | `rc-release-pipeline.yml` on a three-hourly schedule; `nightly-pipeline.yml` and `e2e-gchat-test.yml` by hand | nothing — it gates releases, not pull requests                                                                    |
...
the exception inside the exception: it is in the `nightly-e2e` suite and nothing else, so nothing
