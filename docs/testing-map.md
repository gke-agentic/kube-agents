...
| The release gate                                                                           | `tests/e2e/`                                                                                     | `rc-release-pipeline.yml` three-hourly, `staging-promote.yml` nightly, `e2e-gchat-test.yml` and `e2e-manual-runner.yml` by hand | nothing — it gates releases, not pull requests                                                                    |
...
then runs the whole `nightly-e2e` matrix at 02:00 UTC against the newest validated candidate, and
breaking a test only that matrix reaches costs the night's promotion to staging instead. The manual
`e2e-gchat-test.yml` and `e2e-manual-runner.yml` are further callers, not the only ones.
