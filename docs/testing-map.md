...
| The release gate                                                                           | `tests/e2e/`                                                                                     | `rc-release-pipeline.yml` three-hourly, `staging-promote.yml` nightly, and `e2e-gchat-test.yml` by hand | nothing — it gates releases, not pull requests                                                                    |
...
`e2e-gchat-test.yml` is a third caller, not the only one. `tests/e2e/operator/agentplugins_e2e_test.py`
is reached by the nightly matrix alone: no three-hourly run and no pull request touches it.
