...
### The self-improvement loop is two more separate identities
...
Its Kubernetes grant is the upstream `view` ClusterRole bound with a **RoleBinding**, so it stops at the release namespace, plus read on `platformagents`/`agentplugins` and `get`/`update`/`patch` on one ConfigMap by name — the ledger it keeps its occurrence counts in. `view` excludes Secrets, and `pods/exec`, `pods/attach` and `pods/portforward` are excluded with it, so a read grant cannot become a shell in the agent's pod. There is deliberately no `create` on ConfigMaps: the API server ignores `resourceNames` on create, so any create grant would be a namespace-wide one.

Under `mode: fork` or `mode: upstream` a second identity appears: the KSA `kube-agents-selfimprove-token-minter`, bound to the GSA `kubeagents-selfimprove-minter`, running a self-improvement-only [minty](https://github.com/abcxyz/github-token-minter) Deployment against the loop's **own** GitHub App, not the one `githubMinter` uses for GitOps. Two Apps means the loop's token cannot reach a customer's GitOps repositories and the agent's cannot reach `gke-labs/kube-agents`, and either can be revoked without the other noticing. The rule files scope permissions per repository, and in `upstream` mode no repository gets both: `contents: write` alone on the fork the loop pushes branches to, and `pull_requests: write` alone on the upstream, which therefore never receives a direct push. Opening a cross-fork pull request writes to the base repository rather than the head, so the fork needs no `pull_requests` grant. `fork` mode has one repository playing both roles, and there it carries both. Under the default `report-only` mode neither the minter nor the credential proxy is rendered and the loop has no GitHub identity whatsoever. [`docs/designs/self-improvement.md`](https://github.com/gke-labs/kube-agents/blob/main/docs/designs/self-improvement.md) §3 and §6 are canonical for both.
