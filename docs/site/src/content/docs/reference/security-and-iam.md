...
| ----------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Kubernetes RBAC** (the KSA) | Everything the agent does against a cluster's Kubernetes API      | **No** for workloads and cluster state — read-only in every configuration apart from a specific namespace-confined Role (leader-election), and it cannot read Secrets. Enforced by RBAC.                                             |
...
- **Workloads and cluster state cannot be mutated through the Kubernetes API by this agent.** The KSA's only write grant is the `kubeagents:leader` Role, stringently confined to the agent's own namespace allowing leader-election `leases` and `get`/`patch` on `pods`. Beyond that it holds no write verb (see [Kubernetes RBAC](#kubernetes-rbac)). This holds regardless of any other setting.
...
The Kubernetes `minimal` and `local` roles carry no write verbs (`create`, `update`, `patch`, `delete`) and grant no read access to Secrets or cluster RBAC. (The GCP IAM `read-only` permission set independently provides cluster-viewer read via Cloud IAM for audits connecting through `gcloud container clusters get-credentials`.) The only write grant anywhere in Kubernetes RBAC is the `leader` Role, confined to the agent's own namespace and explicit resource targets: leader-election `leases` and `get`/`patch` on `pods`. The agent cannot modify Deployments, Services, or namespaces, and it cannot read Secret values — if a resource it proposes needs a Secret, it references the Secret by name rather than reading its contents.
