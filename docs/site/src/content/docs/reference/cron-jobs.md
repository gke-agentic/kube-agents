...
| `risk`             | string          | Declared risk tier (`"low"` or `"high"`). High-risk jobs escalate tool approval mode to `deny` during execution, requiring every command to pass policy evaluation. Autonomous watchdogs run as `"low"`, while high-exposure tasks run as `"high"`. Defaults to `"low"`.                                                                                                                       |
