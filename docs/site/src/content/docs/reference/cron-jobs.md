...
| `risk`             | string          | Declared risk tier (`"low"` or `"high"`). For agentic jobs, `"high"` applies a fail-closed read-only command policy (`cron_command_policy_block`), while `"low"` retains the configured mode (denylist floor). Unannotated jobs default fail-closed to `"high"` at runtime. For `no_agent: true` jobs, `"high"` is an input-threat classification label.                                                              |
