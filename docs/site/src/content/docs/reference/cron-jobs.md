...
| `risk`             | string          | Declared risk tier (`"low"` or `"high"`). For agentic jobs, `"high"` applies a fail-closed read-only command policy (`cron_command_policy_block`), while `"low"` retains the configured mode (denylist floor). Runtime-created jobs default to `"low"`, legacy volume entries are backfilled to `"low"` across all profiles, and unannotated in-flight dispatches default fail-closed to `"high"`. For `no_agent: true` jobs, `"high"` is an input-threat classification label.                                    |
