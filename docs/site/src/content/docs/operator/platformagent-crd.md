...
- **`github`** — `org` and `gitRepo`:
  - `org` (optional, up to 39 characters, pattern `^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?$`) — the target GitHub organization or user account. If omitted, it is inferred from the `owner/repo` slug in `gitRepo`.
  - `gitRepo` (optional, up to 2048 characters) — the initial target GitOps repository. Supports HTTPS/HTTP (`https://`, `http://`), SCP-style SSH (`git@...`), SSH/Git protocols (`ssh://`, `git://`), bare `owner/repo` shorthand (e.g. `gke-labs/kube-agents`), or a bare repository name (e.g. `kube-agents`) when `org` is set. Rejects URLs containing whitespace, control characters, or invalid syntax at admission (`failurePolicy: Fail`). The operator cleans the repository slug (`CleanRepoSlugWithOrg`) and seeds it into the `<name>-github-state` ConfigMap (`managed_repos`), ensuring it remains present across reconciles. If an invalid or unparseable value is encountered during reconciliation, a `Degraded` condition (`Reason: InvalidGitRepoURL`) is surfaced on the resource status. Additional repositories can be registered dynamically at runtime via the `register-github-repo` skill. To unregister a CR-declared repository, update or clear `spec.integration.github.gitRepo` on the CR; dynamically registered repositories can be removed directly from the ConfigMap. Populated by `provision_10_deploy_github_minter.sh`.
