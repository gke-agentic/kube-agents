...
- `ConfigMap`s for the pod: config overlays merged into each Hermes profile's `config.yaml` at startup (including the whole rendered config for the default, Chat Agent, profile — see [how config reaches each profile](/kube-agents/operator/platformagent-crd/#how-config-reaches-each-profile)), a `SETTINGS.md` (GKE scope) mounted into `/opt/data/`, the `<name>-github-state` ConfigMap (`managed_repos`) for dynamic GitOps repositories, and a Fluent Bit config for the logging sidecar. Each profile's base config is baked into the image and scaffolded at startup.
- Optional integrations wired through the CR `spec.integration` block: Google Chat (Pub/Sub topic/subscription), Slack (bot/app token secret refs), and GitHub (`org` / `gitRepo`, with the GitHub Token Minter endpoint injected as an env var).
