...
`None` also silences the `hermes_otel` plugin, which carries agent spans. When `OTEL_SDK_DISABLED=true` is set (or `HERMES_OTEL_ENABLED=false` is configured), the container entrypoint disables `hermes_otel` (`enabled: false`) and clears its backends list, so no spans are created or exported to the fallback URL and the gateway logs `[hermes-otel] Telemetry disabled (enabled=false, backends=[])` at start-up. Setting `HERMES_OTEL_ENABLED=true` in `spec.deployment.env` overrides this if spans should be exported regardless.
