...
Ingress paths converge on the agent pod: user chat, scheduled cron, and the operator itself. Inside the pod, one Hermes gateway hosts two co-located profiles: the **Chat Agent** (the `default` profile — the conversational front door and the only chat ingress) and the **Platform Agent** (the `platform` profile — the privileged specialist that does the infrastructure work). They coordinate through a shared kanban board: the Chat Agent files a card, the gateway's dispatcher spawns the Platform Agent as a worker, and the completion is relayed back into the originating chat thread.
...
5. The gateway's kanban dispatcher spawns the **Platform Agent** (`hermes -p platform chat -q "work kanban task <id>"`), which runs the tool-calling loop: system prompt (`SOUL.md`) + card body + available tools. Cluster reads go through the `gke` MCP server; mutations that touch infra route through the `submit-suggestion` skill (see flow 3) rather than direct `kubectl`.
