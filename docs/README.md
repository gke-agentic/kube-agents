...
| `docs/designs/testing-strategy.md` | Feature design | What "tested" means for an autonomous actor with standing authority in a production fleet, and the four tiers that answer it: unit, integration (real seams, fake agent), presubmit evals, release gate, with nightly a merged but unrun pipeline. Carries the domain-coverage rule, the six-rung eval gate ladder, and the statistical treatment of judged scores. | Tiers, domain as unit of coverage, 3 repetitions per case, non-inferiority against `main`'s baseline, pinned judge and pinned scorer, keep writing new cases | **Draft**, in review; §4.2's statistical gate is advisory until baselines exist. Unit tests are real and the integration tier gates; the seeded fleet is applied and standing; presubmit runs two cases and blocks nothing; the release gate is one test; nightly is merged but has no cron and no project yet (§4.4) |
