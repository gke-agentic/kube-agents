...
| `docs/designs/nightly-environment-and-staging-promotion.md` | Feature design | Plan for giving the nightly E2E matrix its own GCP project so it stops sharing a cluster with the three-hourly release pipeline, and for promoting the candidate it passes to staging. Records the current state of both pipelines, the three unrelated meanings of "environment", a phased plan, and the decisions still open. | Nightly environment, generic provisioning scripts, staging promotion tags, naming unification | Plan, not implemented (banner in file); implements `testing-strategy.md` §4.4 |
| `docs/designs/weekly-release-promotion.md` | Feature design | Plan for promoting a staging-gated commit to a GA release on a weekly cron instead of a human clicking "Run workflow": read the newest staging tag, skip when it is already released, halt on a breaking change. Records the promotion ladder, what PR #970 already builds, and the attempt-versus-release cadence decision. | Release cadence, staging-gated releases, breaking-change halt, skip-not-fail | Plan, not implemented (banner in file); depends on the staging gate |
