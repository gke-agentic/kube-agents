...
| Capacity-preflight sizing floor constants and Helm rollout timeout bounds (`DEFAULT_HELM_TIMEOUT`, `HELM_TIMEOUT_MIN_SECONDS`, `HELM_TIMEOUT_MAX_SECONDS`) | `PREFLIGHT_MIN_*` and `HELM_TIMEOUT_*` in `scripts/installer/installer_common.sh` (mirrored in `install.sh` for pre-clone runs); `DEFAULT_HELM_TIMEOUT` in `install.defaults.env` |
