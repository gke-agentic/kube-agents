...
| Release and promotion tag shapes (`rc_*`, `*_validated`, `staging/**`) | `scripts/release/common.sh` (`STAGING_TAG_PREFIX`, `staging_tag_for_rc`) and `scripts/release/resolve_rc_tag.sh` |
...
| `docs/designs/e2e-testing-harness.md` | Feature design | Architecture, execution model, and scenario matrix of the multi-stage E2E testing harness, RC promotion gates, and the nightly matrix that gates promotion to staging. | E2E stages, fleet audit probes, stockout scenarios, live chat, staging promotion gate | Design of record; implemented (banner in file) |
...
| `docs/testing-map.md` | Contributor guide | The mechanics behind `AGENTS.md`'s rule for where a test goes: the nine test homes, what runs each, and how far "runs on a pull request" is from "gates a merge" — two paths-filtered workflows that report green having run nothing, a release gate on three-hourly and nightly schedules rather than manual dispatch, and a directory no `PYTHON_TEST_DIRS` glob reaches. | Test-tier routing, `make test-python`/`test-bench`/`verify`, `dorny/paths-filter` green-on-nothing, RC and staging-promotion schedules, test discovery globs | AI coding agents and human contributors; mechanics only, `AGENTS.md` owns the rule |
...
| `deploy/release-versioning.md` | Site page | How Release Candidate builds are promoted to staging nightly, and to immutable SemVer releases across container images, Helm charts, and TF modules. | Staging promotion, SemVer promotion, chart appVersion, TF git tag ref pinning | SREs and contributors |
...
| `scripts/release/README.md` | Component README | Overview of Release Candidate (RC) pipeline scripts: candidate tag creation (Step 1), environment provisioning (Step 2), GKE readiness & E2E test execution (Step 3), validated tag promotion (Step 4), the nightly staging promotion built on top of them, and the staging-gated release attempt built on top of that. | Release Candidate scripts, RC automation, staging promotion, release gate | CI maintainers and release operators |
