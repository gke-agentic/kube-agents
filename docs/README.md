...
| `install/upgrade.md` | Site page | Moving an existing install to a newer release with `upgrade.sh`: the three modes, how the target version is resolved, previewing, and the refusals that land before anything changes. | Release-pinned one-liner, upgrade modes, tag resolution, refusals | Operators upgrading an install |
