...
| `docs/designs/weekly-release-promotion.md` | Feature design | Plan for promoting a staging-gated commit to a GA release on a weekly cron instead of a human clicking "Run workflow": read the newest staging tag, skip when it is already released, halt on a breaking change. Records the promotion ladder, the resolver's conditions, and the attempt-versus-release cadence decision. | Release cadence, staging-gated releases, breaking-change halt, skip-not-fail | Plan, not implemented (banner in file); depends on the staging gate |
