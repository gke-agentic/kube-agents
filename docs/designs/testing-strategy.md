...
>
> It has not run yet, and a dispatch today does nothing at all: every `rc_*_validated` candidate predates the shared-pipeline restructure, and the resolver refuses those rather than testing them with workflows their trees cannot answer. Clearing that needs the RC pipeline to validate a post-restructure commit; `scripts/release/README.md`, "Workflow Mapping", has the reasoning.
>
