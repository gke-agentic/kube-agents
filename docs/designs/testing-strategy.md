...
>
> It has not run yet, though nothing outside this repository is missing any more: the `kube-agents-nightly` GCP project and the `nightly` GitHub environment that supplies its coordinates both exist, and `scripts/release/README.md`, "The nightly environment", records what they hold. What is left is a cron. The pipeline ships `workflow_dispatch`-only on purpose, so the first run is a deliberate dispatch rather than a schedule firing on merge night, and the schedule is turned on as its own change once that dispatch is boring. **Until it is, nothing runs the full E2E matrix on any schedule.**
>
