...
>
> Once a candidate is eligible, what remains outside this repository is the token minter's GitHub App installed on the nightly GitOps repository with its private key in that project's KMS key, and the environment's own Chat refresh token. Both sit under the matrix's blocking half, so the first real dispatch fails on a fleet-audit or Chat assertion rather than on anything it was meant to grade. The `kube-agents-nightly` GCP project, the `nightly` GitHub environment and its variables and secrets do exist, which is the part that used to be missing. `scripts/release/README.md`, "The nightly environment", is the checklist. Daily execution is scheduled by `nightly-scheduler.yml` (`17 2 * * *`), which resolves candidates and dispatches `nightly-pipeline.yml` only when unpromoted candidates exist.
>
